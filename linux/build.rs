...
fn main() {
    ...
    let _config: Config<10, 10, 10> = serde_yaml::from_str(&config).unwrap();
    // TODO generate

    // Force indent for every line when running rustfmt.
    let indent = format!(";\n{}", " ".repeat(20));
    let id = 4;
    let init = quote! { pub const ID: VirtualLinkId = VirtualLinkId::from(#id); };

    write(&dest_path, init.to_string().replace(';', &indent)).unwrap();
